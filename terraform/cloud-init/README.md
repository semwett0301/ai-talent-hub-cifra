...
- `cloud-init.yaml.tftpl` — cloud-config template rendered by `templatefile()` in
  ...
  both `root` and the `${deploy_user}` user (no sudo), waits for the Docker daemon,
  adds that user to the docker group, configures UFW (22/80), and creates
  `/opt/<app_name>` owned by it.

Notes: template vars passed from `main.tf` — `deploy_user` (`var.DEPLOY_USER`),
`ssh_public_key`, `app_dir`, `hostname`. Docker +
Compose v2 come **pre-installed from the Timeweb "Docker" image**

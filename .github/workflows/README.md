...
- `backend.yml` — Ruff lint + format check + mypy on `backend/` (push/PR, path-filtered).
- `frontend.yml` — oxlint on `frontend/` (push/PR, path-filtered).
- `deploy.yml` — CI/CD on `workflow_dispatch` and push to `main`:
  0. `check-secrets` — fails fast if any secret `infra`/`deploy` needs is missing; the
  ...
  3. `deploy` — reads the server IP from Terraform output (S3 state, no `SSH_HOST`
     ...
     rsyncs the repo to `/opt/<APP_NAME>`, writes `dozzle/users.yml` there from
     `LOGS_USER`/`LOGS_PASSWORD` (hashed on the runner with `dozzle generate`, sent by
     scp), then `docker compose up -d --build`.

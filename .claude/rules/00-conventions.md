...
- Read config only through `common.core.settings.settings` (grouped: `settings.<group>.<field>`, see `70-settings.md`); never `os.environ`.
- Log via `common.core.logging.get_logger`; no `print`.
- Reuse enums from `common.entities.news`; don't duplicate string literals.

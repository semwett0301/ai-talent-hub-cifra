...
- `pyproject.toml` — deps on `domain` + fastapi, uvicorn, aio-pika, httpx (the call
...
- `news_service/`
  ...
  - `deps.py` — **composition root**: builds `NewsRepo`, `NewsIngestor`, and the
    ...
    `get_npa_escalation` (with `HttpNpaGateway` on `settings.npa_service_url`) for the routes.
  ...
  - `infrastructure/` — port implementations: `repositories/` (`NewsRepo`, a session
    ...
    the batching consumer is the shared one in `domain/core/rabbit/`.
...
Escalation (`POST /{id}/npa`, body = `domain.entities.npa.NpaDTO`) runs inside one DB

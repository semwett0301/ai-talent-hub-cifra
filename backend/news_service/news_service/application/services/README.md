...
- `news_service.py` — `NewsService`: read-side use cases over `NewsRepository` —
  `list` (paged, newest first) and `dismiss` (flips `is_alert` to true; returns None
  for an unknown id, which the API maps to 404).
...
of the broker protocol, so it lives in `infrastructure/rabbit`; application only
sees "here is a batch".

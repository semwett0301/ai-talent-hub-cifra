...
- `news_feed.py` — `NewsFeed`: over `NewsRepository` — `list(query)` (items matching the
  filters, one per event cluster so the feed never repeats a story, minus clusters the
  ranker judged of low relevance — not-yet-ranked items still show — newest publication

...
- `event_summary.py` — `EventSummary`: persisted text, `has_primary_event` flag,
  `is_regulatory_alert` flag (the item reports a Russian normative act that reaches the
  company and calls for action — persisted as `news.is_alert`), embedding, `published_at`.
  Immutable — `with_embedding(...)` returns a copy one stage further.

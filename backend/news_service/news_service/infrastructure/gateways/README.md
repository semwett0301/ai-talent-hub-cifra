...
- `npa_http_gateway.py` — `HttpNpaGateway`: implements `NpaGateway` over httpx.
  ...
  `settings.npa_service_url` + `/` with a 10 s timeout and returns the created act's
...
each other: the payload is the shared `domain.entities.npa.NpaDTO`, the wire is HTTP.

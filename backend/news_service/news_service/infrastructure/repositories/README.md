...
- `news_repo.py` — `NewsRepo(session)`: implements the read-API's `NewsRepository` over
  **the one `AsyncSession` it is given** — it opens and closes nothing;
  ...
  `coalesce(published_at, created_at) >= since`, `is_alert` equality when set, `ILIKE` on
  title and text with LIKE wildcards escaped by `_like_pattern`) plus `IS_CLUSTER_HEAD`: one row per event —
  ...
  and, unless `is_alert=true` is asked for (`_gates`), `IS_RANKED_RELEVANT` over the LEFT
  JOINed `news_cluster_ranking` (`contains_eager` into `News.cluster_ranking`): only rows
  whose cluster is ranked `релевантно` / `важно` / `требует внимания` pass — `низкая
  релевантность` and unranked rows do not. `get` has neither filter (any row can be opened by
  id). Writes stage on the session:

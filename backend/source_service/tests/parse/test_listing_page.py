...
from source_service.domain.urls import is_pagination_url, listing_identity
...
def test_listing_links_keep_card_order_and_dates():
    ...
    assert [link.title_hint for link in links] == [
        "Fresh important news story",
        "Old important news story",
    ]
    assert links[0].card_published_at == datetime(
        2026, 9, 4, 12, 0, tzinfo=ZoneInfo("Europe/Moscow")
    )
    assert (

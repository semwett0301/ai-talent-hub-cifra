"""Pure HTML reading of an article page: its metadata and its publication-date signal.

The body text is never scanned for dates: a date mentioned in a story is not the date the
story was published. `DateSource` — the vocabulary of extraction techniques — lives here
because this cascade is what produces it."""
...
from .dates import RU_MONTH_NAMES
...
class DateSource(StrEnum):
    """Which extraction technique produced a publication date, most to least reliable."""
...
def extract_publication_date_signal(html: str | None) -> PublicationDateSignal:
    ...
    if meta.get("jsonld_date_published"):
        ...
        return PublicationDateSignal(
            value, DateSource.JSON_LD, 0.99, f"JSON-LD datePublished={value}"
        )
    ...
    for tag in soup.find_all("meta"):
        ...
        if key == "article:published_time" and content:
            return PublicationDateSignal(
                content, DateSource.OPEN_GRAPH, 0.98, f"article:published_time={content}"
            )
    ...
    for tag in soup.find_all("meta"):
        ...
        if key in reliable_keys and content:
            return PublicationDateSignal(content, DateSource.META, 0.93, f"meta[{key}]={content}")
    ...
    for time_tag in soup.find_all("time", limit=8):
        ...
        if value:
            text = clean_text(time_tag, 200)
            return PublicationDateSignal(value, DateSource.TIME_ELEMENT, 0.88, text or value)
    ...
    if candidates:
        ...
        return PublicationDateSignal(value, DateSource.VISIBLE_DOM, confidence, evidence)
